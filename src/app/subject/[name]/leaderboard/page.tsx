...
const LeaderboardComp = async ({ params }: { params: { name: string}}) => {
	...
	try {
		...
		students.sort((a, b) => b.subjects_with_exp.find(x => x.subject.subject_name == subject_name)?.exp - a.subjects_with_exp.find(x => x.subject.subject_name == subject_name)?.exp)
